...
def load_current_tongchou():
    """用 2026 官方年度覆盖层刷新名单/代码/名额，同时继承旧档案的画像、坐标和历史线。"""
    base = load_json(os.path.join(KB, "2025_sjtongchou_chaoyang.json"))
    overlay = load_yaml(os.path.join(KB, "2026_sjtongchou_chaoyang.yaml"))
    ...
    out.update({
        ...
        "verification_method": overlay["verified"],
        "totals": overlay["totals"],
        "global_confidence": "T1·2026 北京教育考试院官方计划逐页视觉核对",
    })
    for tier_key in ("tongchou_yi", "tongchou_er"):
        old_by_code = {str(r.get("school_code")): r for r in base.get(tier_key, [])}
        rows = []
        for patch in overlay.get(tier_key, []):
            code = str(patch["school_code"])
            rec = dict(old_by_code.get(code) or {})
            rec.update(patch)
            rec["faces_chaoyang"] = int(rec.get("quota_chaoyang") or 0) > 0
            rec["confidence"] = "T1·2026官方计划"
            rec["sources"] = [overlay["source_url"]]
            rec["tongchou_major"] = {
                "major_code": str(patch["major_code"]),
                "major_name": "普通班",
...
# ---------- 2026 首年招生公办校（已有官方统招代码，正式转为公办实体） ----------
for s in new2026:
    pred = pred_by_name.get(s["name"]) or pred_by_code.get("NEW")
    code = str(s.get("code") or "")
    major = str(s.get("major_code") or "")
    au = OrderedDict([("channel", "统招"), ("code", code or None), ("major", major or None),
                      ("major_name", "普通班"),
                      ("campus", "main"),
                      ("plan_total", s.get("plan_total")),
                      ("plan_district", s.get("plan_district")),
    ...
    schools.append({
        "_sort": ("4new", s["name"]),
        "type": "公办普高", "canonical_name": s["name"], "short_name": short_of(s["name"]),
        "aliases": [code] if code else [], "level": s.get("level"), "note": s.get("note"),
        "campuses": [{"slug": "main", "name": "校本部", "lat": s.get("lat"), "lon": s.get("lon"),
                      "address": s.get("address"), "confidence": s.get("confidence"),
                      "boarding": (s.get("boarding") if isinstance(s.get("boarding"), bool) else None)}],
        "admissions": [au],
        "rollup": {
            "features": {"style": s.get("style"), "tags": ["2026首年招生"]},
            "system": s.get("system"), "evidence_refs": s.get("evidence_refs"),
            "new2026_record": s,
        },
        "_codes": [code] if code else [], "_estimate": True,
    })
    ...
    for s in schools:
        ...
        if tot:
            s["admissions"].append(OrderedDict([
                ...
                ("metric", "校内排名"), ("slots_2026_total", tot),
    ...
    for gk, recs in tc_groups.items():
        ...
        for tier_cn, r in recs:
            ...
            au = OrderedDict([
                ...
                ("major", (r.get("tongchou_major") or {}).get("major_code")),
                ("faces_chaoyang", r.get("faces_chaoyang")),
                ("quota_chaoyang", r.get("quota_chaoyang")),
                ("boarding", r.get("boarding"))])
            if lines: au["lines"] = lines
            admissions.append(au)
        # 朝阳本区学校是同一实体的另一录取渠道，必须挂回公办实体，不能另建重复学校。
        if dpy_h == "cy":
            host = next((s for s in schools
                         if str(r0.get("school_code")) in [str(c) for c in s.get("_codes", [])]),
                        None)
            if host is None:
                flag("REVIEW", "tongchou",
                     f"{r0.get('name')}({r0.get('school_code')}) 是朝阳校但未匹配公办实体")
            else:
                host["admissions"].extend(admissions)
            continue
...
# 区级渠道元数据随 registry 一起生成，避免实体已更新而运行时仍读取旧年度。
meta_path = os.path.join(OUT, "_channel_meta.yaml")
channel_meta = load_yaml(meta_path) if os.path.exists(meta_path) else {}
channel_meta["new2026"] = {
    k: v for k, v in load_yaml(os.path.join(KB, "chaoyang_new2026.yaml")).items()
    if k != "schools"
}
channel_meta["tongchou"] = load_current_tongchou()
channel_meta["xeddx"] = load_yaml(os.path.join(KB, "chaoyang_xeddx.yaml"))
with open(meta_path, "w", encoding="utf-8") as f:
    yaml.safe_dump(to_plain(channel_meta), f, allow_unicode=True, sort_keys=False, width=100)
