#!/usr/bin/env python3
"""politics_image_paper — 北京中考道法试卷 PNG → final.json（v1 路线）。

复用 chinese_image_paper 框架（dual-OCR + 5 层防御）+ politics 特化：
  - 25 题 / 70 分 / 70 分钟 / 开卷
  - 4 大题：单选(12×2=24) / 判断(5×2=10) / 材料分析(6×3-4=26) / 写作(1×10=10)
  - 主观题答案是大段中文（材料分析/写作），单选 [A-D]，判断 "正确/错误"
  - 与 politics_docx_paper.py 对齐 type 命名：choice / judge / material / essay

流水线（与 chinese 相同）：
  1. Tencent GeneralAccurate OCR 全 N 页（cache）
  2. ANSWER_MARKER 切题目页 vs 答案页 → 答案页 GeneralBasic 补
  3. _split_sections 切 4 大题
  4. _parse_section_generic 抽 stem + options
  5. _parse_answers 解析答案
  6. _self_check + 输出 yaml

用法:
  TENCENT_OCR_SECRET_ID=... TENCENT_OCR_SECRET_KEY=... \\
    python3 scripts/exam-ocr/politics_image_paper.py \\
      knowledge-original/<series>/<round>/<region>/politics --subject politics
"""
...
# ─── 北京中考道法结构（25 题 / 70 分 / 70 分钟 / 开卷）─────────────────────
# 跨区差异极大（不像 chinese 那样稳定）：
#   西城二模：10 判断 + 10 选择 + 第二部分 5 大题（材料分析含写作） = 25 题/70 分
#   朝阳: 第一部分(判断/选择 20 题) + 第二部分(5 大题) — 题号合并连续 1-25
...
SECTION_BY_CONTENT = [
    ...
    (re.compile(r"^\s*[一二三四五六七][、.]\s*(?:写作|文段写作)"), "essay"),
    # 道法特有：朝阳卷无 "三、材料分析题" header，用"第二部分"切（复用 docx 路线经验）
    (re.compile(r"^\s*第二部分\s*$"), "material"),
]
# 答案页 marker（道法多变体）
ANSWER_MARKER = re.compile(
    r"参考答案|答案及评分|道德与法治(?:试卷)?\s*答案|道法(?:试卷)?\s*答案"
    r"|[一二三四五]模答案|[一二三四五]次练习答案"
    r"|初[一二三]\s*(?:道法|道德与法治)\s*答案",
    re.MULTILINE
)
...
def _detect_figure_bbox_on_page(page_img: Path, hint: str = "") -> tuple[float,float,float,float] | None:
    ...
    if not api_key:
        print(f"[politics_image_paper] ⚠ 无 DASHSCOPE_API_KEY，跳过自动图裁剪", flush=True)
        return None
    try:
        import openai
    except ImportError:
    ...
    try:
        ...
        print(f"[politics_image_paper] ⚠ qwen-vl 定位图失败 ({page_img.name}): {e}", flush=True)
        return None
...
def _crop_figure_to_file(page_img: Path, bbox_pct: tuple, out_path: Path) -> bool:
    ...
    try:
        ...
        if ix2 - ix1 < 50 or iy2 - iy1 < 50:
            print(f"[politics_image_paper]     bbox 退化: ({ix1},{iy1},{ix2},{iy2}) "
        ...
        print(f"[politics_image_paper] ⚠ 裁图失败: {e}", flush=True)
        return False
...
def _auto_crop_figures_for_passages(passages: list, page_texts: list,
    ...
    figs_dir = (repo_root / "knowledge-base" / "exams" / "mock"
                 / "politics" / "beijing" / slug / "figures")
    n = 0
    for ps in passages:
        pid = ps.get("id","?"); ptype = ps.get("type","?")
        # 跳过已有 figure 的（手工 patch 已配置）
        if ps.get("figure"):
            print(f"[politics_image_paper]   passage[{pid}] 已有 figure，跳过", flush=True)
            continue
        body = ps.get("body","") or ""
        # 触发条件：body 含"图1/2/.." 或 passage type 是 non_continuous
        if not (_has_figure_reference(body) or ptype == "non_continuous"):
            print(f"[politics_image_paper]   passage[{pid}] type={ptype} 无图引用 → skip", flush=True)
            continue
        print(f"[politics_image_paper]   passage[{pid}] type={ptype} 触发裁图...", flush=True)
        try:
            # 定位含此 passage 的页
            page = _find_page_for_passage(body, page_texts)
            if not page:
                print(f"[politics_image_paper]   passage[{pid}] 未找到对应页", flush=True)
                continue
            print(f"[politics_image_paper]     找到页: {page.name}", flush=True)
            m = re.search(r"(图[1-5][^\n。]{0,30})", body)
            hint = m.group(1) if m else ""
            print(f"[politics_image_paper]     hint: {hint!r}, 调 qwen-vl...", flush=True)
            bbox = _detect_figure_bbox_on_page(page, hint)
            if not bbox:
                print(f"[politics_image_paper]   passage[{pid}] qwen 未返回有效 bbox",
                      flush=True)
                continue
            print(f"[politics_image_paper]     qwen bbox: {bbox}", flush=True)
            ...
            if _crop_figure_to_file(page, bbox, out_path):
                ...
                n += 1
                print(f"[politics_image_paper]   🔧 自动裁图 passage[{pid}] from "
                      f"{page.name}, bbox={bbox}", flush=True)
            else:
                print(f"[politics_image_paper]   passage[{pid}] PIL 裁图失败", flush=True)
        except Exception as e:
            import traceback
            print(f"[politics_image_paper]   passage[{pid}] 异常: {e}", flush=True)
            traceback.print_exc()
    return n
...
def _ocr_page_basic_supplement(img: Path, cache_dir: Path, force=False) -> str:
    ...
    try:
        ...
        print(f"[politics_image_paper] ⚠ Basic 补 OCR fail ({img.name}): {e}", flush=True)
        return ""
...
def _infer_qtype(n: int, section: str, ans_correct: str, ans_solution: str,
    ...
    """道法题型推断。section 优先（judge/choice/material 直接对应）；
    fallback 用答案形态。
    """
    # **道法 section 直接映射 type**（最高优先级）
    if section in ("judge", "choice", "material"):
        # 选择题：答案是 A-D 单字母
        if section == "choice":
            return "choice"
        # 判断题：答案是 "正确/错误/√/×/对/错"
...
CN_TYPE_LABEL = {
    ...
    "material": "材料分析",
    "essay": "作文",
    # 以下是 chinese 路线遗留 type 名（politics 未用，但保留以免下游崩）
    "dictation": "默写",
    "appreciation": "赏析",
...
def parse_paper(src: Path, out_dir: Path, force=False) -> dict:
    ...
    if answer_start is None:
        ...
        if len(ans_heads) >= 3:
            ...
            print(f"[politics_image_paper] ⚠ ANSWER_MARKER 未命中，"
    ...
    # 道法全 section 都走 _parse_section_generic
    for sec_typ in ("judge", "choice", "material"):
        if sec_typ in sec:
            rng, _ = sec_by_typ[sec_typ]
            _parse_section_generic(sec[sec_typ], rng, sec_typ,
                                     passages, questions, sec_typ)
    ...
    slug = out_dir.name
    print(f"[politics_image_paper] 🔍 auto_crop scan: {len(passages)} passages", flush=True)
    n_cropped = _auto_crop_figures_for_passages(passages, page_texts, out_dir, slug)
    if n_cropped:
        print(f"[politics_image_paper] ✅ 自动裁出 {n_cropped} 张 passage 图表", flush=True)
    else:
...
_EXPECTED_FULL_SCORE = 70   # 道法 70 分
...
def _self_check(result: dict) -> None:
    ...
    # 题数应 22-27（道法 12 区 25 题，跨区可能 ±2）
    if len(qs) < 20:
        fatal.append(f"题数 {len(qs)} < 20（道法最少 20 题）")
    elif len(qs) < 23:
        warn.append(f"题数 {len(qs)} 偏少（北京道法标准 25 题）")
    # 题号连续性
    if nums_have:
    ...
    if abs(actual - _EXPECTED_FULL_SCORE) > 5:
        warn.append(f"分值 {actual} ≠ 期望 ~{_EXPECTED_FULL_SCORE}（差 >5）")
    # 选择题答案空
    for q in qs:
        t = q.get("type", "")
...
def _write_yaml(result: dict, src: Path, out_dir: Path) -> None:
    ...
    if yaml_path.exists():
        try:
            ...
            print(f"[politics_image_paper] ⚠ 读旧 yaml 合并 qc_* 失败: {e}", flush=True)
    ...
    patch_path = repo_root / "knowledge-base" / "exams" / "_patches" / "politics" / f"{slug}.yaml"
    if patch_path.exists():
        try:
            patches = Y.safe_load(patch_path.read_text(encoding="utf-8")) or {}
            applied = _apply_patches(patches, result, yaml_questions)
            if applied:
                print(f"[politics_image_paper] 🔧 应用 {applied} 处人工 patch（{patch_path.name}）",
                      flush=True)
        except Exception as e:
            print(f"[politics_image_paper] ⚠ 加载 patch 失败: {e}", flush=True)
    ...
    yaml_path.write_text(
        ...
        encoding="utf-8")
    print(f"[politics_image_paper] ✅ yaml {yaml_path}", flush=True)
...
def main():
    ...
    try:
        ...
        if patch_path.exists():
            ...
            if applied:
                print(f"[politics_image_paper] 🔧 final.json 应用 {applied} 处 patch", flush=True)
    except Exception as e:
        print(f"[politics_image_paper] ⚠ patch 进 final.json 失败: {e}", flush=True)
    fj = structured / "final.json"
    fj.write_text(json.dumps(result, ensure_ascii=False, indent=2),
                  encoding="utf-8")
    qs = result["questions"]
    print(f"[politics_image_paper] ✅ {fj}", flush=True)
