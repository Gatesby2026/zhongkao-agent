#!/usr/bin/env python3
"""math_image_paper — 北京中考数学试卷 PNG → final.json（v1 image OCR 路线）。 用腾讯云 QuestionSplitOCR 单 API 替换整套 OCR+layout+图分配。
...
用法：
  TENCENT_OCR_SECRET_ID=... TENCENT_OCR_SECRET_KEY=... \\
    python3 scripts/exam-ocr/math_image_paper.py \\
...
# 北京数学一模/二模题型固定结构：Q1-8 单选, Q9-16 填空, Q17-28 解答
...
# 北京数学一模/二模分值默认表（兜底；从答案页"X、Y题各N分" hint 覆盖）
# 标准：单选 2 + 填空 2-3（Q12/Q16 多分） + 解答 5-7 = 100
def _default_score(num: int) -> int:
    if 1 <= num <= 8:   return 2   # 单选 8×2=16
    if 9 <= num <= 16:  return 2   # 填空 8×2=16（部分区个别题 3 分）
    if 17 <= num <= 19: return 5   # 解答 Q17-19
    if num == 20:       return 6
    if 21 <= num <= 22: return 5
    if 23 <= num <= 24: return 6
    if num == 25:       return 5
    if num == 26:       return 6
    if 27 <= num <= 28: return 7   # 压轴 Q27/Q28 各 7
    return 5
...
def _fill_gaps_via_general_ocr(api_results: dict[str, dict],
    ...
    if not missing:
        return []
    print(f"[math_image_paper] 题号缺口 {missing}，用通用 OCR 补抽", flush=True)
...
def main():
    ...
    print(f"[math_image_paper] {len(pages)} 页 → {out_dir}", flush=True)
    ...
    print(f"[math_image_paper] API 总耗时 {time.time()-t0:.1f}s", flush=True)
    ...
    if missing_correct and answer_pages:
        ...
        print(f"[math_image_paper] qwen-vl 兜底补 {len(qd)} 个选择题答案，"
    ...
    if rf:
        print(f"[math_image_paper] 通用 OCR 补 {rf} 题 stem/子小题", flush=True)
    ...
    if fb:
        ...
        print(f"[math_image_paper] fallback 补抽 {len(fb)} 题 → 总 {len(questions)} 题",
              flush=True)
    print(f"[math_image_paper] 解析出 {len(questions)} 题 / {len(answers)} 答案 "
    ...
    if rt:
        print(f"[math_image_paper] OCR 表格补 {rt} 题 stem", flush=True)
    ...
    if nmoved:
        print(f"[math_image_paper] 全局重分配转移 {nmoved} 张图", flush=True)
    ...
    if nfix:
        print(f"[math_image_paper] paddle 兜底 {nfix} 题 figures", flush=True)
    ...
    if not a.no_crop:
        ...
        for q in questions:
            ...
            if fp:
                q["figure_path"] = fp
        print(f"[math_image_paper] 裁切 {len(figure_paths)} 张 figures", flush=True)
    ...
    if score_rules:
        ...
        print(f"[math_image_paper] 答案页解析分值规则覆盖 {len(score_rules)} 题",
              flush=True)
    ...
    final = {
        ...
        "duration_minutes": 120,  # 北京数学 120 分钟（下游 enrich 透传到 yaml）
    ...
    n_patch = load_and_apply_patches(slug, "math", final, repo_root)
    if n_patch:
        print(f"[math_image_paper] 🔧 应用 {n_patch} 处 patch", flush=True)
    ...
    fj.write_text(json.dumps(final, ensure_ascii=False, indent=2),
                  encoding="utf-8")
    print(f"[math_image_paper] ✅ {fj}", flush=True)
