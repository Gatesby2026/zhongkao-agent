#!/usr/bin/env python3
"""english_docx_paper — 北京中考英语 docx 解析版 → final.json (v1)

基于 chinese_docx_paper.py（共享 ~80% 框架）。英语特异性：
  - 满分 60 / 90 分钟 / 38 题（13 区高度统一）
  - 5 大题：单项填空(12×0.5=6) + 完形填空(8×1=8) + 阅读理解(13×2=26) +
    阅读表达(3×2+1×4=10) + 文段表达(1×10=10)
  - 答案：选择题 ABCD 单字母，阅读表达/写作 长文字 sol
  - cloze passage 含 ___N___ 编号填空（完形 8 题共享 1 passage）
  - reading A 篇是 image-match：题号 21-23 嵌入 markdown table cell `___21___`
    格式（需额外 IMG_MATCH_Q_RE 兜底识别）
  - 写作题 Q38 常二选一（题目① + 题目②），第二题独立题号 Q39
  - 题号上限 40（chinese 框架默认 30，必须放宽）
  - sub block total 平摊在 A 篇(3题) vs BCD(10题) 不准 → 全部用 _type_weight，
    不依赖 block total

输入：精品解析 .zip 含 (原卷版.docx + 解析版.docx)
"""
...
SECTION_HEADERS = [
    (re.compile(r"^\s*[一二三四五]\s*[、.]\s*单项?\s*填空"),                "choice"),
    (re.compile(r"^\s*[一二三四五]\s*[、.]\s*完形\s*填空"),                  "cloze"),
    (re.compile(r"^\s*[一二三四五]\s*[、.]\s*阅读\s*理解"),                  "reading"),
    (re.compile(r"^\s*[一二三四五]\s*[、.]\s*阅读\s*(?:与\s*)?表达"),         "reading_express"),
    (re.compile(r"^\s*[一二三四五]\s*[、.]\s*阅读\s*短文"),                   "reading_express"),  # yanshan 别名
    (re.compile(r"^\s*[一二三四五]\s*[、.]\s*(?:文段表达|文段写作|书面表达|写作)"), "essay"),
]
...
def parse_docx_chinese(md: str, figures_dir: Path) -> dict:
    ...
    # **英语 essay 二选一**（朝阳 Q38 intro+题目① 双题号 38）：dedup 同 number essay，
    # 合并 stem 保留最长。Q39 是题目② 备选独立保留。
    seen_essay = {}
    deduped = []
    for q in questions:
        if q.get("section") == "essay":
            n = q["number"]
            if n in seen_essay:
                prev = seen_essay[n]
                prev["stem"] = (prev.get("stem","") + "\n\n" + (q.get("stem","") or "")).strip()
                if not prev.get("solution") and q.get("solution"):
                    prev["solution"] = q["solution"]
                continue
            seen_essay[n] = q
        deduped.append(q)
    questions[:] = deduped
    ...
    for q in questions:
        ...
        for i, ln in enumerate(md_lines):
            ...
            if first_num_match_idx >= 0:
                q["_line_idx"] = first_num_match_idx
            else:
                # **英语 image-match 题号** 在 `___N___` 形式（markdown table cell）
                IMG_Q_RE = re.compile(rf"_+\s*{n}\s*_+")
                for i, ln in enumerate(md_lines):
                    if IMG_Q_RE.search(ln):
                        q["_line_idx"] = i
                        break
                else:
                    q["_line_idx"] = 0
...
def _parse_section(sec_typ: str, sec_lines: list[str],
    ...
    for i, ln in enumerate(sec_lines):
        ...
        # 凡是 reading section 内含 `_+N_+` 且 N 不在 q_starts 都识别为题号
        if sec_typ == "reading":
            for m in re.finditer(r"_+\s*(\d{1,2})\s*_+", ln):
                n = int(m.group(1))
                if 1 <= n <= 40 and n not in {qs[1] for qs in q_starts}:
                    q_starts.append((i, n, ln))
...
STANDARD_SECTION_TOTAL = {
    ...
    "cloze":           8,   # 8 × 1
    "reading":        26,   # 13 × 2
    "reading_express": 10,   # 3 × 2 + 1 × 4
    "essay":          10,   # 1 × 10（或 2 题二选一）
}
...
def _assign_types(questions: list[dict]) -> None:
    ...
    for q in questions:
        ...
        elif sec == "cloze":         q["type"] = "cloze"
        elif sec == "reading":       q["type"] = "reading"
        elif sec == "reading_express": q["type"] = "reading_express"
        elif sec == "essay":         q["type"] = "essay"
        else:
            ans = q.get("answer", "") or ""
...
def _type_weight(qtype: str, n: int):
    """英语题型固定分值（13 区统一）。"""
    return {
        "choice":          0.5,   # 单项填空 12 × 0.5 = 6
        "cloze":           1,     # 完形 8 × 1 = 8
        "reading":         2,     # 阅读 13 × 2 = 26
        "reading_express": 2,     # 阅读表达 3×2 + 末题 4 = 10
        "essay":           10,    # 作文 1 × 10
    }.get(qtype, 1)


def _allocate_scores_in_block(qs_in_block: list[dict], total: int) -> None:
    """英语：完全用 _type_weight 算分，不依赖 block total。
    原因：sub block 平摊在 A 篇(3题) vs BCD(10题) 不准（26→13/13 → BCD 10×2=20 > 13）。
    各题型分值在 13 区一致，按 type 计算最稳。
    """
    if not qs_in_block: return
    # essay 二选一（朝阳 Q38 题目① + Q39 题目②）：第一题 10 分，备选 0 分 + 注明
    if all(q["type"] == "essay" for q in qs_in_block):
        for i, q in enumerate(qs_in_block):
            if i == 0:
                q["score"] = 10
            else:
                q["score"] = 0
                cur_sol = q.get("solution", "") or ""
                if "二选一" not in cur_sol and "任选" not in cur_sol:
                    q["solution"] = f"[二选一备选题目，与上一题任选其一作答；本题不重复计分]\n{cur_sol}".strip()
        return
    fixed_score = {}
    for i, q in enumerate(qs_in_block):
        fixed_score[i] = _type_weight(q["type"], q["number"])
    # reading_express 末题 +2（Q37=4，13 区统一）
    if all(q["type"] == "reading_express" for q in qs_in_block) and len(qs_in_block) >= 2:
        fixed_score[len(qs_in_block) - 1] = 4
    for i, q in enumerate(qs_in_block):
...
def main():
    ...
    ap.add_argument("--out-root", type=Path,
                     default=Path("/Users/jiakui/projects/zhongkao-agent/knowledge-base/exams/_staging/english"))
    a = ap.parse_args()
    src = a.src.resolve()
    if not src.exists():
    ...
    print(f"[english_docx_paper] 用解析版: {docx.name}", flush=True)
    ...
    # **patches 系统**（与 image v3 路线共用 _patches/english/<slug>.yaml）
    ...
    qs = result["questions"]
    print(f"[english_docx_paper] ✅ {fj}", flush=True)
    print(f"   slug: {slug}")
...
def _apply_patches(patches: dict, result: dict) -> int:
    ...
    schema 同 image 路线 _patches/english/*.yaml：
...
def _populate_exam_meta(result: dict, slug: str, src_stem: str) -> None:
    ...
    result["duration_minutes"] = 90    # 北京中考英语 90 分钟
