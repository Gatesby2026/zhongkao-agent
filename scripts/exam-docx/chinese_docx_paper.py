...
SECTION_HEADERS = [
    (re.compile(r"^\s*一\s*[、.]\s*基础\s*[··]?\s*运用"),     "base"),
    (re.compile(r"^\s*二\s*[、.]\s*(?:古诗文|诗文)\s*阅读"),  "classical"),
    (re.compile(r"^\s*三\s*[、.]\s*(?:名著|整本书)\s*阅读"),  "book_review"),
    (re.compile(r"^\s*四\s*[、.]\s*现代文\s*阅读"),           "modern"),
    (re.compile(r"^\s*五\s*[、.]\s*(?:作文|写作|文段表达)"),  "essay"),
]
