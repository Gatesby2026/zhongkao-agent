#!/usr/bin/env python3
"""chinese_docx_paper — 北京中考语文 docx 解析版 → final.json

输入：精品解析 .zip 含 (原卷版.docx + 解析版.docx) 或单 解析版.docx
处理流程：
  1. 解压 zip / 直接打开 docx
  2. 优先用 解析版.docx（含题面 + 【答案】 + 【N题详解】 + 【点睛】译文）
  3. body 顺序遍历 paragraphs + tables；run 层抽 <w:em w:val="dot"/> → ·X·、
     <w:u w:val="single"/> → <u>X</u>
  4. 切五大题（一二三四五）+ 资料一/二/三 + (一)(二)(三) + 后记 + 材料一/二/三
  5. 题号锚 ^N. 切题；首个 "【答案】" 触发 q_text/a_text 切分
  6. 答案抽取：
     - "【答案】1. X    2. Y    3. Z" 多题合并行
     - "N. X" 或 "N. 示例：..." 续行
     - "【N题详解】<solution>" 单题详解
     - "【点睛】参考译文：..." 古诗文译文 → 拼到 passage.body
  7. 表格 → markdown table（Q14 名著 5×3）
  8. 内嵌 PNG 抽到 figures/
  9. 输出 final.json，与 image 版同 schema

跟 docx_paper.py（数学路线）共享 _load_docx / _extract_image / _walk_table 等底层；
但 run 层 walker 重写以加入加点字/下划线 + 跳过 OMML（语文无公式）。
"""
...
SECTION_HEADERS = [
    (re.compile(r"^\s*[一二三四五]\s*[、.]\s*基础\s*[··]?\s*运用"),     "base"),
    (re.compile(r"^\s*[一二三四五]\s*[、.]\s*(?:古诗文|诗文)\s*阅读"),  "classical"),
    (re.compile(r"^\s*[一二三四五]\s*[、.]\s*(?:名著|名者|整本书)\s*阅读"),  "book_review"),
    (re.compile(r"^\s*[一二三四五]\s*[、.]\s*现代文\s*阅读"),           "modern"),
    (re.compile(r"^\s*[一二三四五]\s*[、.]\s*(?:作文|写作|文段表达)"),  "essay"),
]
...
def _allocate_scores_in_block(qs_in_block: list[dict], total: int) -> None:
    ...
    if any(q["type"] == "essay" for q in qs_in_block):
        essays = [q for q in qs_in_block if q["type"] == "essay"]
        for k, q in enumerate(essays):
            if k == 0:
                q["score"] = 40
            else:
                q["score"] = 0
    ...
    for i, q in enumerate(qs_in_block):
        if q["type"] == "handwriting":
            ...
            fixed_score[i] = 2
        elif q["type"] == "dictation":
            # 默写按 ①②③ 空数估
            sol = q.get("solution","") or ""
            blanks = max(1, sum(sol.count(c) for c in "①②③④⑤⑥⑦⑧"))
            fixed_score[i] = blanks  # 每空 1 分
        else:
            flex_idx.append(i)
...
def main():
    ...
    ap.add_argument("--out-root", type=Path,
                     default=Path("/Users/jiakui/projects/zhongkao-agent/knowledge-base/exams/_staging/chinese"))
    a = ap.parse_args()
    src = a.src.resolve()
    if not src.exists():
    ...
    qs = result["questions"]
    print(f"[chinese_docx_paper] ✅ {fj}", flush=True)
    print(f"   slug: {slug}")
