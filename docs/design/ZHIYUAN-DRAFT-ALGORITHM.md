...
- **通勤(跟随住宿勾选,与统招同口径)**:`prefillTongchou` 增加 `reachByCommute(s.dist.km, s.boarding)` 过滤——`≤上限 || (接受住宿 && 该校提供住宿)`。统筹多为外区远校(6~72km),**远校无住宿(没法住校又通勤不了)→排除**;**未勾住宿时统筹可能几乎为空**(只剩 ≤上限 的极少数),这是诚实结果。下拉(`tcEligible`)仍列全、显示 km/🛏,可手动加。
...
- 应用于:③`bandPool`(含够不上回填)、校额`prefillXed`、**统筹`prefillTongchou`**(三处同口径,跟随住宿勾选)。
- **下拉永远列全**(`selectable`/校额/统筹下拉不过滤,可手动选远校;统筹下拉显示 km/🛏)。
- 统筹特殊性:本质外区远校,未勾住宿时按此规则会几乎为空(诚实结果)。
...
| D8 | 统招/校额/统筹**通勤口径统一**为 `reachByCommute`(跟随住宿勾选);住宿默认勾选、通勤上限不置灰 | 三批次一致;勾住宿只放开"本身提供住宿"的远校;上限始终生效 |
| D9 | `unified.py` 外区统筹校 name 追加 `·校区` | 人大附/清华附 本部(统筹一)+分校区(统筹二)同名→schools_unified id 撞键、查学校重复、详情面板串号 |
| D10 | **每条记录定 uid + 地图改用 schools_unified 驱动 + 详情按 uid 解析** | 根除地图标记↔结构化记录的名字匹配脆弱性(D9 回归的根因)。uid=招生编码(公办/民办)或 type:name(统筹/中职/贯通/新校);公办坐标/地图属性后端从 result.points 补入 unified;无结构化的6所民办补兜底点 |
