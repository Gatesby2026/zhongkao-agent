...
knowledge-original/<series>/<round>/<region>/<subject>/images/page-*.png   [raw]
  ...
  ▼
knowledge-base/mock-exams/<subject>/beijing/<slug>/                        [derived staging]
  ├─ pages/  layout-cache/  structured-cloud/final.json+md  figures/
  │  scripts/exam-ocr/extract_figures.py     (paddle 图形)
  │  scripts/exam-ocr/qc_report.py           (质量门禁)
  │  scripts/knowledge-base/enrich_to_mock_exam.py
  ▼
knowledge-base/mock-exams/<subject>/beijing/<slug>.yaml                    [最终知识库件]
  │
...
| `knowledge-base/mock-exams/` | 试卷结构化（主力，3500+ 文件） | track | per-卷 staging + `<slug>.yaml`；其中 `pages/`+`layout-cache/` 为可重生缓存→gitignore，只 track `structured-cloud/`+`figures/`+`.yaml` |
| `knowledge-base/question-banks/` | 题库（289 文件） | track | 主力子域 |
| `knowledge-base/`{study-guides,common-mistakes,diagnostics,exam-analysis,learning-paths,subjects} | 知识库各子域 | track | 在用，中等体量 |
| `knowledge-base/`{assessment,resources,regions,workbooks,admission} | 近空骨架 | track | **待决**：补内容 / 并入 / 归档（阶段 4） |
| `students/` | 学生答题卡+小分（真实姓名/成绩） | ignore | 私；`<name>/<slug>/`；`_web/`=server 运行时（暂留） |
| `data/` | `answer-card-poc/` 等运行时 POC（exams/ 与 zhongkao.db 已归档） | ignore | legacy 已清，仅留 POC |
| `out/` | **唯一交付根**：`student-reports/<id>/<slug>/report.{md,pdf}`、`papers/<slug>.pdf` | student-reports/ + papers/ ignore | ✅ 已收口 |
| `archived/legacy-*` | `legacy-data-exams/`(39 旧件) + `legacy-db/`(旧 sqlite) | ignore | 备查不入 git |
