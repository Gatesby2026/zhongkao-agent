...
**阶段 A — 单引擎（当前可用，准确率 ~70%）**：
- 直接调 Qwen-VL-Max + 严格 prompt
- 提示模型逐题判断、不要重排
- 适用：手写答案部分（Q16+）

**阶段 B — 涂卡专用 CV pipeline（待开发，目标准确率 >98%）**：
```
1. 检测答题卡边界 + 矫正透视（OpenCV findContours + warpPerspective）
2. 模板匹配：定位单选题区 / 多选题区 / 大题区
3. 单选/多选格子定位（基于固定栅格）
4. 每个格子计算"填涂度"（灰度平均值 vs 阈值）
5. 输出 ABCD 每格是否涂黑
```

预估工作量：阶段 B 需 5-7 天 + 多套真实答题卡训练数据校准。

**短期降级方案**：
- 选择题让家长**手工核对系统识别结果**（小程序界面上勾选）
- 大题部分用 Qwen-VL OCR 即可（不需要 >98%）
