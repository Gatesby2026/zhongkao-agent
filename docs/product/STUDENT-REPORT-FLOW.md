...
### P0 已收口
- [x] **EXIF 方向根因修复（2026-05-18）**：手机照片横置+EXIF=6，流水线零
  EXIF 处理 → 选择题识别错/截图区域错/截图方向歪。修复：上传落盘前
  `imgnorm` 正立化去 EXIF + 技能本体 crop_subjective/detect 统一正立。
  用用户真实旋转照片 POST 生产端到端验证：14 选择题干净、11/11 主观题
  正确裁切（宽>高）、报告 57/70 逐题具体错因（Q18 长木板 等基于事实）。
- [x] 真机等价验证：用户原始旋转照片 POST 生产 `/api/analyses` 全程跑通，
  报告非失真（仍建议用户自行从手机再走一遍最终确认体验）
