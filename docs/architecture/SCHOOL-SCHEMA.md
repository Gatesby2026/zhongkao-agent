...
| `line_trend` | object/null | **录取位次历史趋势**（uid 命中 `ts/line_trend.json`）`{ ranks{23,24,25}, latest, volatility, volatile }`。T3·仅历史+波动。**2026 预估唯一权威源 = `pred_2026`（朝阳口径）**；旧 `ref_2026*` 已废除（曾与 pred_2026 并列展示两个相差很大的 2026 数字，审计 P1-9）。 |
