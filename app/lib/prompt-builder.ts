...
1. **整体分析**（1段即可，简明扼要）：当前水平、与目标的差距、核心策略方向。不要展开论述。

2. **各模块一句话诊断**（表格形式，每模块一行：模块名 | 当前水平 | 关键问题 | 必拿分题号 | 过关标准）

3. **完整周计划**（共${diagnosis.weeksUntilExam}周，这是整份报告的核心）：

   **第1周（本周）**要求非常详细，精确到每天：
   - 每天的学习模块、时长
   - 具体任务：教材哪一节（如"人教版八下§19.1 p.45-52"）、教辅哪几题（如"五三A版 p.78 第5-15题"）、模拟题哪道（如"2025西城一模第22题"）
   - 每天结尾标注：✅ 完成标准（如"12题对10题以上"）
   - 周五安排限时检测（从已有模拟题中选题组卷）
   - 周末安排错题回顾 + 过关检测（用真题验证本周成果）

   **第2-3周**按周概述：
   - 每周的学习重点模块和目标
   - 关键任务（教材章节 + 教辅专题 + 推荐模拟题）
   - 周末过关检测题目

   **第4周及以后**按阶段概述：
   - 每2-3周为一个阶段，说明阶段目标和重点
   - 最后1-2周标注为"考前冲刺"：全真模拟 + 错题回顾 + 心态调整

   周计划编排原则：
   - 每天交叉安排不同模块（防疲劳、防遗忘）
   - 优先级高的模块每周安排更多时间（参考上面的时间分配比例）
   - 模拟题按难度递进：前期做基础+中档题，后期做压轴题
   - 每周至少1次限时练习（培养考试节奏）

4. **考场策略**（简要，5行以内：时间分配、做题顺序、放弃哪些题）

5. **提醒**（3条以内，最关键的）

整体篇幅控制：分析和策略部分（第1、2、4、5项）合计不超过报告的30%，周计划（第3项）占70%以上。周计划才是学生真正需要的东西。`;
