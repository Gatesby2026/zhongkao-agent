...
├── ...
├── page-NN.jpg
└── <小分>.xlsx       # 可选：老师小分表（teacher_xlsx 场景需要）
```
...
| Case | 学生 | 试卷 | 备注 |
|------|------|------|------|
| `jiaxiaoqi-physics` | 贾小淇 | 朝阳一模 物理 | 缺字母法 baseline 59-60/70 |
| `jiaxiaoqi-math` | 贾小淇 | 朝阳一模 数学 | |
| `jiaxiaoqi-chinese` | 贾小淇 | 朝阳一模 语文 | |

### 生产真实 case（从 https://zhongkao.gatesby.xyz 拉取）

| Case | 学生 | 试卷 | 来源 aid | 用途 |
|------|------|------|---------|------|
| `guanlihan-haidian-physics-er` | 关丽涵 | 2026 海淀二模 物理 | `9cf5379f8880` | **Path B 标准回归**（57/70，选择题 15/15） |
| `jiaxiaoqi-haidian-physics-er` | 贾小淇 | 2026 海淀二模 物理 | `d939a5a862f2` | 海淀方括号格式 cross-validation |
| `zhangjingqi-haidian-physics-er` | 张靖奇 | 2026 海淀二模 物理 | `5c2eb2b03cd3` | 海淀方括号 同区不同学生 |
| `zhangyiran-shijingshan-chinese-yi` | 张伊冉 | 2026 石景山一模 语文 | `341037ff62d3` | 跨区 + 跨学科（语文） |
| `tuominde-chaoyang-physics-yi` | 脱敏的 | 2026 朝阳一模 物理 | `9ccb20332f23` | 学生名脱敏的真实卡 |
...
1. `guanlihan-haidian-physics-er` — Path B 海淀代表
2. `jiaxiaoqi-physics` — 缺字母法 朝阳代表

两个分数 ±2 才能合并到 main。

完整回归再加 3 个生产 case：

3. `zhangjingqi-haidian-physics-er`
4. `zhangyiran-shijingshan-chinese-yi`（跨学科）
5. `tuominde-chaoyang-physics-yi`
