            ...
            <template v-if="selectedPoint">
              <template v-if="selSchool">
                ...
                <div v-if="selSchool.pred_2026" class="dp-block dp-pred">
                  ...
                  <div class="dp-line dp-muted">{{ selSchool.pred_2026.method === 'new_anchor' ? '新校锚定' : selSchool.pred_2026.method === 'tongchou_cy_equiv' ? '跨区位次映射(外区线→朝阳口径)' : selSchool.pred_2026.method === 'hist' ? ('以' + (selSchool.pred_2026.base_year || '最近年') + '录取位次为参考（非模型预测·网传/历史线）') : selSchool.pred_2026.method === 'supply_stack_x_fenduan' ? '双锚法(2026官方分数段×供给栈)' : selSchool.pred_2026.method === 'supply_stack' ? '供给栈法(2026官方名额结构×2025位次锚)' : '百分位法' }} · 可信度 {{ selSchool.pred_2026.conf }} · 2026官方分数段折算；各校真实录取线须等录取结束后确认</div>
