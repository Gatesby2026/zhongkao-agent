            ...
            <template v-if="selectedPoint">
              <template v-if="selSchool">
                ...
                <div v-if="selSchool.pred_2026" class="dp-block dp-pred">
                  ...
                  <div v-if="selSchool.pred_2026.score" class="dp-line">折算分数：<b class="dp-predv">≈{{ selSchool.pred_2026.score }}分</b><span class="dp-muted" v-if="selSchool.pred_2026.score_lo && selSchool.pred_2026.score_hi"> · 区间 {{ selSchool.pred_2026.score_lo }}–{{ selSchool.pred_2026.score_hi }}分</span></div>
                  <div class="dp-line dp-muted">{{ selSchool.pred_2026.method === 'new_anchor' ? '新校锚定' : selSchool.pred_2026.method === 'tongchou_cy_equiv' ? '跨区位次映射(外区线→朝阳口径)' : selSchool.pred_2026.method === 'hist' ? ('以' + (selSchool.pred_2026.base_year || '最近年') + '录取位次为参考（非模型预测·网传/历史线）') : selSchool.pred_2026.method === 'weighted_supply' ? '供给/分流权重模型(2025实线主锚)' : selSchool.pred_2026.method === 'conservative_fill' ? '保守填报口径(2025实线主锚×2026官方分数段校准)' : selSchool.pred_2026.method === 'supply_stack_x_fenduan' ? '双锚法(2026官方分数段×供给栈)' : selSchool.pred_2026.method === 'supply_stack' ? '供给栈法(2026官方名额结构×2025位次锚)' : '百分位法' }} · 可信度 {{ selSchool.pred_2026.conf }} · 2026官方分数段折算；各校真实录取线须等录取结束后确认</div>
