...
async function loadHistory() {
  try { history.value = (await api.list()).items || [] } catch { history.value = [] }
}
...
async function openHistory(h: { id: string; status: string }) {
  if (h.status !== 'done') return
  try {
    report.value = await api.report(h.id)
    analysisId.value = h.id
    step.value = 4
  } catch (e: any) {
...
<div class="app-shell">
    ...
    <div v-show="step===0" class="scroll-area">
      ...
      <!-- 我的历史报告（登录后留存）-->
      <div class="card hist-card" v-if="history.length">
        <div class="section-title" style="margin-bottom:8px">我的历史报告</div>
        <div v-for="h in history" :key="h.id" class="hist-item"
             :class="{ clickable: h.status === 'done' }" @click="openHistory(h)">
          <div class="hist-main">
            <div class="hist-name">{{ h.student_name || '（未命名）' }}</div>
            <div class="hist-meta">{{ fmtDate(h.created_at) }}</div>
          </div>
          <span v-if="h.status === 'done'" class="hist-go">查看 ›</span>
          <span v-else-if="h.status === 'failed'" class="hist-tag fail">失败</span>
          <span v-else class="hist-tag">进行中</span>
        </div>
      </div>
...
.hist-name { font-size:14px; font-weight:600; color:var(--gray-900,#111827); }
.hist-meta { font-size:11.5px; color:var(--gray-500,#6b7280); margin-top:2px; }
.hist-go { font-size:13px; color:var(--brand,#2563eb); font-weight:600; white-space:nowrap; }
.hist-tag { font-size:11px; padding:2px 8px; border-radius:999px;
  background:var(--gray-100,#f3f4f6); color:var(--gray-500,#6b7280); white-space:nowrap; }
.hist-tag.fail { background:#fdecea; color:#c0392b; }
